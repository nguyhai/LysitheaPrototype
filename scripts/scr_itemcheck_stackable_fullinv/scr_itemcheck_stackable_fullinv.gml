...
for (i = 0; i < 60; i++) { // Looping through inventory
	...
	if (global.inventory[i] == argument0) and global.inventory[i+1, 9] < 100 and pickedup = false { // If index and stackcount of found item is same
...
var zerocheck = stackcount-remainder;
 
if remainder > 0 and zerocheck > 0{
	...
	for (i = 0; i < 30; i++){
		...
		if global.showPickup[i] = -1 {
			...
			global.showPickup[i+1,0] = name; 
			global.showPickup[i+1,1] = upgradelevel; 
			global.showPickup[i+1,2] = (stackcount-remainder);
			return(remainder);
		}	
	}
